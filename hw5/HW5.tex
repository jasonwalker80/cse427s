...
\item[1.] 
	\begin{itemize}
	\item[a.] The data format is a flat text file where each record represents an HTTP protocol request served by the server generating the log file taking requests from hosts whose IP is recorded withint the log record. The information stored in each record is:\\
		\begin{itemize}
		\item IP Address
